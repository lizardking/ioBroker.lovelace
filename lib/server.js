...
const VERSION = fs.readFileSync(`${getRootPath()}version.txt`, 'utf8').replace(/(\d{4})(\d{2})(\d{2})\.(\d).*/s, '$1.$2.$3');
...
class WebServer {
    constructor(options) {
        ...
        //If the ignoreCustomIds option is set, this array will contain the ids of datapoints that have custom lovelace settings
        //The content of this is is checked during automatic entity creation.
        this._customIdsToIgnore = [];
    ...
    async _readAllEntities() {
        //Must run first since it sets the custom ids to ignore (if they are to be ignored)
        await this._getAllEntities(); //creates manual entities.

        //Must run second since it requires custom ids to ignore 
    ...
    async _getAllEntities() {
        try {
            ...
            if (this.adapter.config.ignoreCustomIds) {
                this._customIdsToIgnore = customids;
            } else {
                this._customIdsToIgnore = [];
            };
     ...
     * @param {string} [forcedEntityId] possible forced entity ID on a conflict case.
        ...
        if (Object.keys(this._objectData.objects).length < 10) {
            try {
                ...
                if (this.adapter.config.aliasOnly) {
                    ...
                    let aliasPattern = 'alias.0.';
                    if (this.adapter.config.aliasPattern && this.adapter.config.aliasPattern.length > 0) {
                        aliasPattern = this.adapter.config.aliasPattern;
                    };


                    params.startkey = aliasPattern;
                    params.endkey = aliasPattern + '\u9999';
                }
                ...
                if (_devices && _devices.rows) {
                    for (let i = 0; i < _devices.rows.length; i++) {
                        if (_devices.rows[i].value && _devices.rows[i].value._id && !ignoreIds.find(reg => reg.test(_devices.rows[i].value._id)) && !this._customIdsToIgnore.includes(_devices.rows[i].value._id)) {
                ...
                if (_channels && _channels.rows) {
                    for (let i = 0; i < _channels.rows.length; i++) {
                        if (_channels.rows[i].value && _channels.rows[i].value._id && !ignoreIds.find(reg => reg.test(_channels.rows[i].value._id)) && !this._customIdsToIgnore.includes(_channels.rows[i].value._id)) {
                ...
                if (_states && _states.rows) {
                    for (let i = 0; i < _states.rows.length; i++) {
                        if (_states.rows[i].value && _states.rows[i].value._id && !ignoreIds.find(reg => reg.test(_states.rows[i].value._id)) && !this._customIdsToIgnore.includes(_states.rows[i].value._id)) {
                ...
                if (_enums && _enums.rows) {
                    for (let i = 0; i < _enums.rows.length; i++) {
                        if (_enums.rows[i].value && _enums.rows[i].value._id && !this._customIdsToIgnore.includes(_enums.rows[i].value._id)) {
    ...
    _init() {
        ...
        this._app.use('/adapter/', async (req, res) => {
            req.url = req.url.replace(/^\/([a-zA-Z0-9-_]+)\//, '/$1.admin/');
            await this._sendFile(req, res);
        });
        ...
        this._app.get('/api/person/*', async (req, res) => {
    ...
    async _initConnection(ws) {
        ...
        ws.on('message', async message => {
            ...
            if (message.type === 'subscribe_events') {
                ...
                this._getCurrentUser(ws)
                    .then(data => this._sendResponse(ws, message.id, data));
    ...
    _markForUpdate(id, obj, entity) {
        if (entity) { //we have entity -> keep only on change for one entity, because it will be completely rebuilt.
    ...
    async onObjectChange(id, obj) {
        if (obj) {
            if (obj.type === 'state' || obj.type === 'channel' || obj.type === 'device') {
                ...
                if (!this.adapter.config.aliasOnly || id.startsWith(aliasPattern) || (obj.common && obj.common.custom && obj.common.custom[this.adapter.namespace])) {
                    this._objectData.objects[id] = obj;
